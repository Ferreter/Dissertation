...
"""Massive API and local-storage helpers used by the dissertation notebooks.

I keep these reusable operations outside the notebooks so that downloading,
normalising and registering market data follows the same rules in every stage.
"""
...
class MassiveAPIError(RuntimeError):
    """Raised when a Massive request cannot be completed safely."""
        ...
        for attempt in range(self.max_retries + 1):
            try:
                ...
                if response.status_code == 429:
                    # I respect the server delay when it is supplied and otherwise
                    # back off gradually so a rerun does not flood the API.
...
def register_parquet_views(con, data_root: Path | str) -> list[str]:
    """Register only datasets that already contain Parquet files."""
